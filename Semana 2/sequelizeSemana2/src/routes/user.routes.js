...
const User = require ('../models/users.model'); //! Se esta impotando el modelo = user.models.js (AHI vienen la tabla creada).
...
router.get('/api/v1/users', async (req, res) => {          // ? Esto es ifual a un  SELECT * FROM User.
    try {
        ...
        res.json(users);    // JSON:stringify(users). ASI SE DEVUELVE LA RESPUESTA. 
    } catch (error) {
